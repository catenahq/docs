...
#
# Multi-stage Astro+Starlight static build -> nginx:alpine serving dist/.
...
# ---- Stage 2: serve ----
# nginx-unprivileged runs as UID 101 (non-root), satisfying trivy DS-0002.
# A non-root user cannot bind ports < 1024, so the listener moves to 8080.
